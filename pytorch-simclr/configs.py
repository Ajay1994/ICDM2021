...
def get_datasets(dataset, augment_clf_train=False, add_indices_to_data=False, num_positive=None):
    ...
    CACHED_MEAN_STD = {
        ...
        'NIHxray': ((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
    }
...
#     ])
    ...
    transform_train = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(*CACHED_MEAN_STD["imagenet"]),
    ])
    ...
    if dataset == 'cifar100':
        ...
        trainset = NIHBiaugment(data_frame = data_frame, transform = transform_train)
        
        clf_data_frame = data_frame.sample(frac=1).reset_index(drop=True)
        
#         clf_data_frame = data_frame.loc[data_frame['label'] != "No Finding"]
#         clf_data_frame = clf_data_frame.sample(frac=1).reset_index(drop=True)
        print("Length of data : {}".format(len(clf_data_frame)))
        train_df = clf_data_frame[: int(0.8 * len(clf_data_frame))]
        test_df = clf_data_frame[int(0.8 * len(clf_data_frame)) :]
        
        testset, clftrainset, num_classes, stem = None, None, 15, StemImageNet
        testset = NIH_CLFAugment(data_frame = test_df, transform = transform_test)
        clftrainset = NIH_CLFAugment(data_frame = train_df, transform = transform_clftrain)
