...
class NIHBiaugment(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        path, target = row["image"], row["label"]
...
class NIH_CLFAugment(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        label_dict = {"Atelectasis": 0, "Cardiomegaly": 1, "Effusion": 2, "Infiltration": 3, "Mass": 4, "Nodule": 5, "Pneumonia": 6, "Pneumothorax": 7,  "Consolidation": 8, "Edema": 9, "Emphysema": 10, "Fibrosis": 11, "Hernia": 12, "Pleural_Thickening": 13, "No Finding":14}
        
        row = self.data_frame.iloc[index]
        path, target = row["image"], label_dict[row["label"].split("|")[0]]
        pil_img = Image.open(path).convert("RGB")
