...
parser.add_argument('--dataset', '-d', type=str, default='NIHxray', help='dataset',
                    choices=['cifar10', 'cifar100', 'stl10', 'imagenet', "NIHxray"])
...
testloader = torch.utils.data.DataLoader(testset, batch_size=512, shuffle=False, num_workers=args.num_workers,
                                         pin_memory=True)
clftrainloader = torch.utils.data.DataLoader(clftrainset, batch_size=512, shuffle=False, num_workers=args.num_workers,
                                             pin_memory=True)


def initialize_net(net):
    trained_net = models.resnet50(pretrained=True)
    trained_weights = {}
    for name, parameter in trained_net.named_parameters():
        trained_weights[name] = parameter
    with torch.no_grad():
        for name, param in net.named_parameters():
            if name in trained_weights:
                print(name + " Initialized ...")
                param.copy_(trained_weights[name])
            if name.replace("stem.", "") in trained_weights:
                print(name + " Initialized ...")
                param.copy_(trained_weights[name.replace("stem.", "")])
# Model
