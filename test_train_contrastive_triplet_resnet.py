...
def train_model(model, criterion, optimizer, scheduler, critic, c_criterion, c_optimizer, epoch):
    ...
    for batch_idx, (image_name, inputs, labels) in t:
        ...
        with torch.set_grad_enabled(True):
            ...
            total_loss = (0.1 * c_loss + 0.9 * loss)
            total_loss.backward()
...
def main():
    fopen = open("accuracyII.txt", "w")
    model_ft = model = ResidualNet('ImageNet', 50, 8, "TripletAttention")
    critic = LinearCritic(2048, temperature=0.5)
    
    #initialize_model(model_ft)
    model_ft.load_state_dict(torch.load("./checkpoint/model_BCE.pth"))
    critic.load_state_dict(torch.load("./checkpoint/critic_BCE.pth"))
    ...
    c_optimizer = optim.SGD(list(model_ft.parameters()) + list(critic.parameters()), lr=0.0000001, weight_decay=1e-6,
                           momentum=0.9)

    #     c_lr_scheduler = lr_scheduler.StepLR(c_optimizer, step_size=1, gamma=0.1)
    ...
    for epoch in range(1, num_epochs + 1):
        model_ft, critic = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler, critic, c_criterion, c_optimizer, epoch)
        auc = eval_model(model_ft, fopen, epoch)
        if auc > best_auc:
            best_auc = auc
#             torch.save(model_ft.state_dict(), "./checkpoint/model_BCEII.pth")
#             torch.save(critic.state_dict(), "./checkpoint/critic_BCEII.pth")
        fopen.flush()
